...
class ExpenseTypes extends Model
{
  ...
  */
  ...
  {
  		if(isset($data['id'])) {
				$this->rules['type_of_expense'] = $this->rules['type_of_expense'].','.$data['id'].',id,is_active,1';
			} else {
				$this->rules['type_of_expense'] = $this->rules['type_of_expense'].',NULL,id,is_active,1';
			}
     ...
     */
		public function insertOrUpdate($data) {
			...
			if(isset($data['id'])) {
				$id = $data['id'];
				unset($data['id']);
				\Session::flash('message', trans('message.expense_type_update_success'));
