...
class TextLocalHelper
{
  ...
  private function _sendRequest($command, $params = array())
  {
    ...
    else return ['invalid' => true, 'msg' => 'Invalid request handler.'];
        ...
        foreach ($result->errors as $error) {
          switch ($error->code) {
            default:
              ...
              return ['invalid' => true, 'msg' => $error->message];
          }
        }
      }
    }*/
  ...
  private function _sendRequestCurl($command, $params)
  {
    ...
    if ($rawResponse === false) {
      ...
      return ['invalid' => true, 'msg' => 'Failed to connect to the Textlocal service: ' . $error];
    } elseif ($httpCode != 200) {
      //throw new Exception('Bad response from the Textlocal service: HTTP code ' . $httpCode);
      return ['invalid' => true, 'msg' => 'Bad response from the Textlocal service: HTTP code ' . $httpCode];
    }
  ...
  private function _sendRequestFopen($command, $params)
  {
    ...
    return ['invalid' => true, 'msg' => 'Unsupported transfer method'];
  }
  ...
  public function sendSms($numbers, $message, $sender, $custom = null, $sched = null, $test = true, $receiptURL = null, $optouts = false, $simpleReplyService = false)
  {

    if (!is_array($numbers)) {
      //throw new Exception('Invalid $numbers format. Must be an array');
      return ['invalid' => true, 'msg' => 'Invalid $numbers format. Must be an array'];
    }
    if (empty($message)) {
      //throw new Exception('Empty message');
      return ['invalid' => true, 'msg' => 'Empty message'];
    }
    if (empty($sender)) {
      //throw new Exception('Empty sender name');
      return ['invalid' => true, 'msg' => 'Empty sender name'];
    }
    if (!is_null($sched) && !is_numeric($sched)) {
      //throw new Exception('Invalid date format. Use numeric epoch format');
      return ['invalid' => true, 'msg' => 'Invalid date format. Use numeric epoch format'];
    }
  ...
  public function bulkJSONApiTest()
  {
    $messages = array(
      'messages' => array(
          array(
    ...
    return $this->_sendRequest('bulk_json', $data);
  }
  ...
  public function bulkJSONApi($messages)
  {
